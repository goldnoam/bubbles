    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-between z-50 text-white p-6">
      ...
      {(isPlaying || isPaused || state === GameState.LEVEL_UP) && (
        <div className="w-full flex justify-between pointer-events-auto">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="bg-black/40 dark:bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 text-right-custom shadow-lg min-w-[140px]">
      ...
      <div className="flex-1 flex items-center justify-center w-full">
        {state === GameState.START && (
          <div className="bg-black/60 dark:bg-black/80 backdrop-blur-xl p-10 rounded-3xl border border-white/20 text-center max-w-md pointer-events-auto shadow-2xl scale-in">
            ...
            {/* Search Flavor Component with Autocomplete & Clear */}
            <div className="relative mb-6 text-left-custom" onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
              <div className="flex gap-2">
                ...
                {searchValue && (
                  <button 
                    onClick={() => handleAction(() => setSearchValue(""), "Cleared")} 
                    className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center hover:bg-red-500/20 transition-colors"
                  >
              ...
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <div className="flex bg-white/10 rounded-lg border border-white/20 p-1">
                  {(['small', 'medium', 'large'] as FontSize[]).map(s => (
                    <button 
                      key={s} 
                      onClick={() => handleAction(() => onFontSizeChange(s), `Font size ${s}`)} 
                      className={`px-2 py-0.5 rounded text-xs transition-colors ${fontSize === s ? 'bg-emerald-500 text-white' : 'hover:bg-white/10'}`}
                    >
                      {s[0].toUpperCase()}
                    </button>
                  ))}
                ...
                <button 
                  onClick={() => handleAction(onToggleTheme, isDark ? "Light theme" : "Dark theme")} 
                  className={`p-2 rounded-lg border border-white/20 hover:bg-white/20 transition-all ${isDark ? 'bg-white/10' : 'bg-white/30 text-yellow-500'}`}
                >
      ...
      {/* Footer with Copyright and Feedback */}
