...
import { GameState, BubbleData, Level, BubbleType } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateVisualization = () => {
      ...
      if (containerRef.current) {
        // We apply a scale and brightness pulse to the container's pseudo-elements or specific layers
  ...
  const spawnBubble = useCallback(() => {
    ...
    const newBubble: BubbleData = {
      ...
      size,
      speed,
      color: currentLevel.bubbleColor,
      type
    };
  ...
  const handlePop = useCallback((bubble: BubbleData) => {
    ...
    switch (bubble.type) {
      ...
      case BubbleType.BOMB:
        triggerShake();
        setBubbles(prev => {
          setScore(s => s + Math.floor(prev.length * 0.7) + 1);
          return [];
        });
        break;
  ...
  const toggleMute = () => {
    ...
    setIsMuted(newMuted);
    if (!newMuted && state === GameState.PLAYING) {
      audioService.playMusic();
    }
  };
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative w-full h-screen overflow-hidden transition-colors duration-1000 ${currentLevel.bottleColor} ${isShaking ? 'screen-shake' : ''}`}
      style={{
        '--beat-intensity': '0',
        '--beat-scale': '1',
        '--beat-brightness': '1'
    ...
    >
      {/* Beat Visualizer Background Layers */}
      <div 
      ...
      {isSlowed && (
        <div className="absolute inset-0 pointer-events-none bg-blue-500/10 z-10 transition-opacity duration-1000"></div>
      )}
